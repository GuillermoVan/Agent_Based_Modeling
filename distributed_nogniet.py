        ...
        for i in range(1,4): # Communicate 3 time steps ahead
            ...
            while avoidance == False:   # Only continue when collision is avoided
                ...
                if agent_1.path[timestep] == agent_2.path[timestep] or (agent_1.path[timestep] == agent_2.path[timestep-1] and agent_1.path[timestep-1] == agent_2.path[timestep]):
                    ...
                    if agent_1.id > agent_2.id and (agent_2.path.count(agent_2.path[time]) <= 4 and agent_2.path[time] != agent_2.goal): # Agent with  highest agent id has priority
                    ...
                    elif agent_2.id > agent_1.id and (agent_1.path.count(agent_1.path[time]) <= 4 and agent_1.path[time] != agent_1.goal):
                        ...
                        change = True
                    print(constraints)
                    print("pad 1",agent_1.path)
                    print("pad 2", agent_2.path)

                    # if agent_1.path.count(agent_1.path[time]) > 4 and agent_1.path[time] != agent_1.goal:
                    #     for constraint in constraint_temp_1:
                    ...
                    # for constraint in constraints:
                    #     # print("c", constraints)
                    #     if constraint['agent'] == prioritized_agent:
                    #         print("c", constraint['loc'])
                    #         locations.add(tuple(constraint['loc']))
                    # if len(locations) > 1:
                    #     if prioritized_agent == agent_1.id:
                    #         for constraint in constraint_temp_2:
                    #             if constraint in constraints:
                    #                 constraints.remove(constraint)
                    #         agent_1.path = agent_1.path[:time] + path_1
                    ...
                    #                 constraints.append(constraint)
                    #
                    #     elif prioritized_agent == agent_2.id:
                    #         for constraint in constraint_temp_1:
                    #             if constraint in constraints:
                    #                 constraints.remove(constraint)
                    #         agent_2.path = agent_2.path[:time] + path_2
